...
public class ManterAtividadeSBean {
	...
	public void cadastrarAtividade(TOAtividade to) {
		Atividade atividade = new Atividade(getId(), to.getNome(), to.getDiaSemana(),  to.getHorarioInicio(), to.getHorarioFim(),to.getProfessor());
	...
	public List<TOAtividade> listarAtividades() {
		...
		for(Atividade user: atividades) {
			tos.add(new TOAtividade(user.getId(), user.getNome(), user.getDiaSemana(),user.getHorarioInicio(), user.getHorarioFim(), user.getProfessor()));
		}
		return tos;
	}
